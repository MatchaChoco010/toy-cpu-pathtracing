...
impl RgbTexture {
    ...
    /// RGB値をスペクトラムに変換する。
    pub fn sample_spectrum(&self, uv: Vec2, spectrum_type: super::config::SpectrumType) -> Spectrum {
        ...
        match spectrum_type {
            super::config::SpectrumType::Albedo => {
                let color = color::ColorSrgb::<color::tone_map::NoneToneMap>::new(texture_rgb[0], texture_rgb[1], texture_rgb[2]);
                spectrum::RgbAlbedoSpectrum::<color::ColorSrgb<color::tone_map::NoneToneMap>>::new(color)
            }
