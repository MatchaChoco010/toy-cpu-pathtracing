...
use scene::{CreatePrimitiveDesc, EmissiveMaterial, FloatParameter, LambertMaterial, RgbTexture, Scene, SceneId, SpectrumParameter, SpectrumType, TextureConfig};
...
fn create_emissive(spectrum: spectrum::Spectrum, intensity: f32) -> scene::Material {
    EmissiveMaterial::new(SpectrumParameter::constant(spectrum), FloatParameter::constant(intensity))
}
...
pub fn load_scene_3<Id: SceneId, F: Filter>(scene: &mut Scene<Id>, camera: &mut Camera<F>) {
    ...
    // テクスチャ付きのbunny
    let geom = scene.load_obj("./renderer/assets/bunny.obj");
    let bunny_material = create_lambert_texture("./renderer/assets/bunny-material-0/BaseColor.png");
    scene.create_primitive(CreatePrimitiveDesc::GeometryPrimitive {
        geometry_index: geom,
