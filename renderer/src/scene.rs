...
use scene::{
    ...
    MetalType, NormalParameter, NormalTexture, RgbTexture, Scene, SceneId, SpectrumParameter,
    SpectrumType,
};
...
pub fn load_scene_6<Id: SceneId, F: Filter>(scene: &mut Scene<Id>, camera: &mut Camera<F>) {
    ...
    scene.create_primitive(CreatePrimitiveDesc::GeometryPrimitive {
        geometry_index: geom,
        surface_material: MetalMaterial::new(MetalType::Gold, NormalParameter::none()),
        transform: Transform::identity(),
    });
