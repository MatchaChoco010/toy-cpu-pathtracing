...
#[derive(Clone)]
pub struct SrgbRendererNee<'a, Id: SceneId, F: Filter, SF: SamplerFactory, T: ToneMap> {
    args: RendererArgs<'a, Id, F, SF>,
    tone_map: T,
    exposure: f32,
    max_depth: usize,
}
impl<'a, Id: SceneId, F: Filter, SF: SamplerFactory, T: ToneMap> SrgbRendererNee<'a, Id, F, SF, T> {
    /// 新しいsRGBレンダラーを作成する。
    pub fn new(
        args: RendererArgs<'a, Id, F, SF>,
        tone_map: T,
...
impl<'a, Id: SceneId, F: Filter, SF: SamplerFactory, T: ToneMap> Renderer
    for SrgbRendererNee<'a, Id, F, SF, T>
{
