...
use crate::f64::eotf::{Eotf, Gamma2_2, Gamma2_6, GammaRec709, GammaSrgb, Linear, NonLinearEotf};
use crate::f64::gamut::{
    ColorGamut, GamutAces2065_1, GamutAcesCg, GamutAdobeRgb, GamutDciP3D65, GamutRec2020,
    GamutSrgb, xy_to_xyz,
};
use crate::f64::tone_map::{InvertibleToneMap, NoneToneMap, ToneMap};
...
impl Xyz {
    /// XYZからLabに変換する関数。
    pub fn xyz_to_lab(&self) -> glam::DVec3 {
        let xyz = self.xyz;
        fn f(t: f64) -> f64 {
            if t > 0.008856 {
                t.powf(1.0 / 3.0)
    ...
    pub fn xyz_to_rgb<G: ColorGamut>(&self) -> ColorImpl<G, NoneToneMap, Linear> {
        let xyz = self.xyz;
        let gamut = G::new();
        let rgb = (gamut.xyz_to_rgb() * xyz).max(glam::DVec3::splat(0.0));
        ColorImpl::create(rgb, gamut, NoneToneMap)
    }
}
impl From<glam::DVec3> for Xyz {
    /// XYZ値を持つXyzを生成する。
    fn from(xyz: glam::DVec3) -> Self {
        Self { xyz }
    }
}
...
pub trait Color: Sync + Send + Clone {
    /// RGB値を取得する。
    fn rgb(&self) -> glam::DVec3;

    /// XYZ値を取得する。
    fn xyz(&self) -> glam::DVec3;
}
...
impl<G: ColorGamut, T: ToneMap, E: Eotf> ColorImpl<G, T, E> {
    /// Colorを生成する。
    fn create(rgb: glam::DVec3, gamut: G, tone_map: T) -> Self {
        Self {
            rgb,
            gamut,
            tone_map,
...
impl<G: ColorGamut, E: Eotf> ColorImpl<G, NoneToneMap, E> {
    ...
    pub fn new(r: f64, g: f64, b: f64) -> Self {
        Self::create(glam::dvec3(r, g, b), G::new(), NoneToneMap)
    }

    /// RGB値を持つColorを生成する。
    pub fn from_vec3(rgb: glam::DVec3) -> Self {
